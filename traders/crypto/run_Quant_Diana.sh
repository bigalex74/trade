#!/bin/bash
# Изолированный воркер для крипты: Quant_Diana

LOG_FILE="/home/user/logs/traders/crypto_Quant_Diana.log"
...
while true; do
    echo "[ $(date '+%Y-%m-%d %H:%M:%S') ] Tick: Quant_Diana (Crypto)" >> "$LOG_FILE"
    
    # Вызов трейдера с таймаутом (рынок крипты открыт всегда)
    timeout 300 $PYTHON_BIN $SCRIPT "Quant_Diana" >> "$LOG_FILE" 2>&1
