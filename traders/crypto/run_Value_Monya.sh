#!/bin/bash
# Изолированный воркер для крипты: Value_Monya

LOG_FILE="/home/user/logs/traders/crypto_Value_Monya.log"
...
while true; do
    echo "[ $(date '+%Y-%m-%d %H:%M:%S') ] Tick: Value_Monya (Crypto)" >> "$LOG_FILE"
    
    # Вызов трейдера с таймаутом (рынок крипты открыт всегда)
    timeout 300 $PYTHON_BIN $SCRIPT "Value_Monya" >> "$LOG_FILE" 2>&1
