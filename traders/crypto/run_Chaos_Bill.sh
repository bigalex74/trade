#!/bin/bash
# Изолированный воркер для крипты: Chaos_Bill

LOG_FILE="/home/user/logs/traders/crypto_Chaos_Bill.log"
...
while true; do
    echo "[ $(date '+%Y-%m-%d %H:%M:%S') ] Tick: Chaos_Bill (Crypto)" >> "$LOG_FILE"
    
    # Вызов трейдера с таймаутом (рынок крипты открыт всегда)
    timeout 300 $PYTHON_BIN $SCRIPT "Chaos_Bill" >> "$LOG_FILE" 2>&1
