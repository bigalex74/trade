#!/bin/bash
# Изолированный воркер для крипты: VSA_Victor

LOG_FILE="/home/user/logs/traders/crypto_VSA_Victor.log"
...
while true; do
    echo "[ $(date '+%Y-%m-%d %H:%M:%S') ] Tick: VSA_Victor (Crypto)" >> "$LOG_FILE"
    
    # Вызов трейдера с таймаутом (рынок крипты открыт всегда)
    timeout 300 $PYTHON_BIN $SCRIPT "VSA_Victor" >> "$LOG_FILE" 2>&1
