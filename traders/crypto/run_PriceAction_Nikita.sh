#!/bin/bash
# Изолированный воркер для крипты: PriceAction_Nikita

LOG_FILE="/home/user/logs/traders/crypto_PriceAction_Nikita.log"
...
while true; do
    echo "[ $(date '+%Y-%m-%d %H:%M:%S') ] Tick: PriceAction_Nikita (Crypto)" >> "$LOG_FILE"
    
    # Вызов трейдера с таймаутом (рынок крипты открыт всегда)
    timeout 300 $PYTHON_BIN $SCRIPT "PriceAction_Nikita" >> "$LOG_FILE" 2>&1
