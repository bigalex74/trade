#!/bin/bash
# Изолированный воркер для крипты: Scalper_Kesha

LOG_FILE="/home/user/logs/traders/crypto_Scalper_Kesha.log"
...
while true; do
    echo "[ $(date '+%Y-%m-%d %H:%M:%S') ] Tick: Scalper_Kesha (Crypto)" >> "$LOG_FILE"
    
    # Вызов трейдера с таймаутом (рынок крипты открыт всегда)
    timeout 300 $PYTHON_BIN $SCRIPT "Scalper_Kesha" >> "$LOG_FILE" 2>&1
